...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, num_sweeps=10, filter_zero=True):
    from nuscenes.utils.geometry_utils import transform_matrix
...
def create_nuscenes_infos(root_path, version="v1.0-trainval", num_sweeps=10, filter_zero=True):
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, train_scenes, val_scenes, test, num_sweeps=num_sweeps, filter_zero=filter_zero
    )

    if test:
        print(f"test sample: {len(train_nusc_infos)}")
        with open(
            root_path / "infos_test_{:02d}sweeps_withvelo.pkl".format(num_sweeps), "wb"
        ) as f:
        ...
        with open(
            root_path / "infos_train_{:02d}sweeps_withvelo_filter_{}.pkl".format(num_sweeps, filter_zero), "wb"
        ) as f:
            pickle.dump(train_nusc_infos, f)
        with open(
            root_path / "infos_val_{:02d}sweeps_withvelo_filter_{}.pkl".format(num_sweeps, filter_zero), "wb"
        ) as f:
