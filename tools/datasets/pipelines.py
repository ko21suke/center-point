...
class LoadPointCloudFromFile(object):
    ...
    def __call__(self, res, info):
        ...
        if self.type == "NuScenesDataset":
            ...
            num_sweeps = res["lidar"]["num_sweeps"]
            ...
            assert (num_sweeps - 1) == len(
                info["sweeps"]
            ), "num_sweeps {} should equal to list length {}.".format(
                num_sweeps, len(info["sweeps"])
            )

            for i in np.random.choice(len(info["sweeps"]), num_sweeps - 1, replace=False):
                sweep = info["sweeps"][i]
            ...
            if num_sweeps > 1:
                ...
                assert (num_sweeps - 1) == len(
                    info["sweeps"]
                ), "num_sweeps {} should be equal to the list length {}.".format(
                    num_sweeps, len(info["sweeps"])
                )
...
__all__ = {
    "LoadPointCloudFromFile": LoadPointCloudAnnotations,
        ...
        for transform in transforms:
            if isinstance(transform, dict):
                if transform['type'] == 'Empty':
                    continue
                # print(transform)
                self.transforms.append(__all__[transform['type']])
            else:
